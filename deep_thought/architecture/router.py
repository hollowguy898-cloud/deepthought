...
Decoupled Routing (revised):
  - Gradient flow through gates is enabled during TRAINING so that
    the router and experts receive gradients from the RL loss.
  - During INFERENCE, gates are detached to save memory (no gradient
    needed).
  - The "slow router update" concept is implemented via a separate
    optimizer with a lower learning rate for router parameters, NOT
    by detaching gates (which killed ALL gradient flow to the router).
"""
...
class NoisyTopKRouter(nn.Module):
    ...
    def forward(
        ...
        By default, gate values have gradient flow enabled during training
        so the router and experts can learn. Set detach_gates=True only
        during inference to save memory.
        ...
        Args:
            ...
            detach_gates: Whether to detach gate values (default False for training; set True for inference)
        ...
        """
        # Ensure inputs are 2D (batch, dim) — squeeze out any extra seq dim
        if h_t.dim() == 3:
            h_t = h_t.squeeze(1)
        if x_t.dim() == 3:
            x_t = x_t.squeeze(1)
        ...
        # Detach gates only when explicitly requested (e.g. during inference).
        # During training, gradient flows through gates so the router learns.
        if detach_gates:
            gates = gates.detach()
...
class AdaptiveRouter(nn.Module):
    ...
    def forward(
        ...
        By default, gradient flows through gates and adaptation so the
        router and adapter can learn during training. Set detach_gates=True
        only during inference to save memory.
        ...
        Args:
            ...
            training: Whether in training mode
            detach_gates: Whether to detach gates (default False for training)
        ...
        if context is None:
            # Ensure 2D for cat
            _h = h_t.squeeze(1) if h_t.dim() == 3 else h_t
            _x = x_t.squeeze(1) if x_t.dim() == 3 else x_t
            _m = m_t.squeeze(1) if m_t.dim() == 3 else m_t
            combined = torch.cat([_h, _x, _m], dim=-1)
        ...
        # Detach adaptation only when explicitly requested (e.g. inference)
        if detach_gates:
        ...
        # Detach gates only when explicitly requested (e.g. inference)
        if detach_gates:
            gates = gates.detach()
...
class SparseRouter(nn.Module):
    ...
    def forward(
        ...
        By default, gradient flows through gates during training so
        the router can learn. Set detach_gates=True for inference.
        """
